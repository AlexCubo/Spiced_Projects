...
def clean_data(df):
    ...
    # by the looks of (any state != checkout) --> NaN. We assume they are not 
    # thieves but just clients taht arrived too late for shopping. We use the function
    # replace_thief_entry(df) to take int account the behaviour of those clients
    # that hang in the same state until they are forced by the security to checkout.
